...
- [2026-05-20] **Published preview empty overlay variants (`TrustCenterContent.tsx` > `PublishedEmptySkeleton`, `trustCenterProfileUrl.ts`):** Three center cards when nothing is visitor-live. **Cold start** (`!hasEverPublished`, no draft): `no-results.png`, title **There is no published Trust Center**, profile URL copy, **Learn More** + **Contact Support**. **Ready to publish** (draft exists, never published): `public/Imagery/ready-to-publish-done.png` (no spaces in path for temp.md), title **Ready to Publish?**, body **Publish your Trust Center draft to trust.mediacore.com**, **Learn More** + **Publish Draft**. **Unpublished** (had been live): prior republish copy + **Contact Support** / **View Draft**. **Right panel** (`PublishedEmptyState`): cold start uses profile URL + **Account verification is pending approval.**; with draft uses **`trust.mediacore.com`** + publish-draft helper. **temp.md:** https://light-isle-d28f.temp.md (update via `.tempmd` + `npm run build && node scripts/update-temp-md.mjs`).
...
- [2026-05-20] **Header and Identity grip overlapped eye icon (`RightPanel.tsx` > `FixedHeaderSections`):** `FixedSectionTooltip` wrapped the grip in `relative inline-flex`, so `absolute left-0` on the grip positioned against the tooltip span in the flex row (on top of the eye) instead of the row gutter. **Fix:** outer span uses `SECTION_LAYOUT_GRIP_GUTTER_CLASS`; tooltip wraps only the centered 20×20 hit target inside the gutter (same structure as draggable page section rows).
